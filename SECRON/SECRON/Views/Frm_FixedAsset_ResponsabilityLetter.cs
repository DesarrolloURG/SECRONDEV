...
{
    public partial class Frm_FixedAsset_ResponsabilityLetter : Form
    {
        #region PropiedadesIniciales

        public Mdl_Security_UserInfo UserData { get; set; }
        public int PreselectedEmployeeId { get; set; } = 0;

        private HashSet<int> _categoriasTecnologiaIds = new HashSet<int>();
        ...
        public Frm_FixedAsset_ResponsabilityLetter()
        {
            InitializeComponent();
        }
        ...
        private void Frm_FixedAsset_ResponsabilityLetter_Load(object sender, EventArgs e)
        {
            ...
            EstadoInicial();
            CargarCategoriasTecnologia();
            PreseleccionarEmpleado();
        }
        ...
        #region ExclusionTecnologia

        private void CargarCategoriasTecnologia()
        {
            try
            {
                ...
                if (clasificaciones == null || clasificaciones.Count == 0) return;

                var categorias = Ctrl_FixedAssetCategories
                    .ObtenerCategoriasPorClasificacion(clasificaciones[0].ClassificationId);

                if (categorias == null) return;

                _categoriasTecnologiaIds = new HashSet<int>(categorias.Select(c => c.Key));
            }
            catch
            {
                _categoriasTecnologiaIds = new HashSet<int>();
            }
        }
        ...
        private void PreseleccionarEmpleado()
        {
            if (PreselectedEmployeeId <= 0) return;

            foreach (var item in ComboBox_Employee.Items)
            {
                if (item is KeyValuePair<int, string> kvp && kvp.Key == PreselectedEmployeeId)
                {
                    ComboBox_Employee.SelectedItem = item;
                    return;
                }
            }
        }
        ...
        private void CargarActivosPorEmpleado(int employeeId)
        {
            try
            {
                ...
                if (activos != null && _categoriasTecnologiaIds.Count > 0)
                    activos = activos.Where(a => !_categoriasTecnologiaIds.Contains(a.AssetCategoryId)).ToList();
        ...
        private void ImprimirCarta(PrintPageEventArgs e)
        {
            ...
            if (!_encabezadoGlobalImpreso)
            {
                ...
                string titulo = "CARTA DE RESPONSABILIDAD DE ACTIVOS FIJOS";
                ...
                string cuerpo =
                    ...
                    $"manifiesto haber recibido los activos fijos descritos a continuación " +
