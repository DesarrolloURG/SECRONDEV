...
{
    public partial class Frm_Checks_Managment : Form
    {
        ...
        // Control de cheques para rango de cheques en impresión
        private int? _checkControlIdActual;
        ...
        private async void Frm_Checks_Managment_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        // Constructor de cheques
        public Frm_Checks_Managment()
        {
            InitializeComponent();
            InicializarImpresion();
            authController = new Ctrl_Security_Auth();
        }
        private void Frm_Checks_Managment_FormClosing(object sender, FormClosingEventArgs e)
        {
            // Detener el timer al cerrar el formulario
        ...
        private void ConfigurarPlaceHoldersTextbox()
        {
            ...
            ConfigurarPlaceHolder(Txt_Concepto, "CONCEPTO DEL CHEQUE");
            ...
            ConfigurarPlaceHolder(Txt_ValorImpresoCheque, "0.00");
        ...
        private void ConfigurarComboBoxes()
        {
            ...
            ComboBox_TipoCheque.DropDownStyle = ComboBoxStyle.DropDownList;
            ComboBox_Location.DropDownStyle = ComboBoxStyle.DropDownList;

            ComboBox_TipoCheque.Items.AddRange(new object[]
            {
                "HOSPEDAJES",
                "PUBLICIDAD",
                "SERVICIOS",
            ...
            ComboBox_TipoCheque.SelectedIndex = 0;
            ...
            // Evento al cambiar tipo de cheque
            ComboBox_TipoCheque.SelectedIndexChanged += ComboBox_TipoCheque_SelectedIndexChanged;
        ...
        // Evento al cambiar tipo de cheque
        private void ComboBox_TipoCheque_SelectedIndexChanged(object sender, EventArgs e)
        {
            string tipoSeleccionado = ComboBox_TipoCheque.SelectedItem?.ToString() ?? "";
            ...
            switch (tipoSeleccionado)
            {
                ...
                case "LIQUIDACIONES":
                case "BONO NAVIDEÑO":
                    // Sin exención, SIN stamps, SIN ISR, CON Indemnización y Vacaciones y Aguinaldo
                    CheckBox_Exencion.Checked = false;
                    CheckBox_Exencion.Enabled = false;
        ...
        private void CalcularValores()
        {
            try
            {
                ...
                string tipoChecked = ComboBox_TipoCheque.SelectedItem?.ToString() ?? "";
                ...
                // Calcular suma de valores impresos de complementos si LastComplement esta activo
                ...
                if (CheckBox_LastComplement.Checked && CheckBox_Complemento.Checked && !string.IsNullOrWhiteSpace(Txt_Complemento.Text))
                {
                    ...
                    try
                    {
                        // Obtener el cheque principal
                        Mdl_Checks chequePrincipal = Ctrl_Checks.ObtenerChequePorNumero(numeroComplemento);
                        if (chequePrincipal != null)
                        {
                            sumaComplementos += chequePrincipal.PrintedAmount;
                        }

                        // Obtener todos los cheques vinculados
                        List<Mdl_Checks> chequesComplemento = Ctrl_Checks.ObtenerChequesComplemento(numeroComplemento);
                        if (chequesComplemento != null && chequesComplemento.Count > 0)
                        {
                            foreach (var cheque in chequesComplemento)
                            {
                                sumaComplementos += cheque.PrintedAmount;
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        System.Diagnostics.Debug.WriteLine($"Error al obtener complementos: {ex.Message}");
                    }
                }
                ...
                Txt_ValorImpresoCheque.Text = _valorImpreso.ToString("N2");
        ...
        private void LimpiarCalculos()
        {
            ...
            Txt_ValorImpresoCheque.Text = "0.00";
        ...
        private void Btn_SearchCuenta_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Frm_Checks_SearchCuenta frmBuscar = new Frm_Checks_SearchCuenta(this);
        ...
        private void CheckBox_LastComplement_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (CheckBox_LastComplement.Checked)
            {
                ...
                decimal valorImpreso = ObtenerValorDecimal(Txt_ValorImpresoCheque);
                if (valorImpreso < 0)
                {
                    MessageBox.Show(
        ...
        private void Txt_Complemento_Leave(object sender, EventArgs e)
        {
            ...
            string numeroCheque = Txt_Complemento.Text.Trim();

            // VALIDACIÓN 1: Solo números
            if (!int.TryParse(numeroCheque, out int _))
            {
                MessageBox.Show(
                    "DEBE INGRESAR UN NÚMERO DE CHEQUE VÁLIDO",
                    "VALIDACIÓN",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
                Txt_Complemento.Text = "";
            ...
            if (!Ctrl_Checks.ValidarExistenciaCheque(numeroCheque))
            {
                MessageBox.Show(
                    $"EL CHEQUE NO. {numeroCheque} NO EXISTE EN LA BASE DE DATOS.\n" +
            ...
            // VALIDACIÓN 3: Verificar si el cheque ya tiene LastComplement = 1
            if (Ctrl_Checks.ValidarLastComplement(numeroCheque))
            {
                MessageBox.Show(
                    $"EL CHEQUE NO. {numeroCheque} YA FUE MARCADO COMO COMPLETADO.\n" +
                    "NO SE PUEDEN AGREGAR MÁS COMPLEMENTOS A ESTE CHEQUE.\n\n" +
                    "El proceso de complementos para este cheque ha sido cerrado.",
                    "CHEQUE COMPLETADO",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
                Txt_Complemento.Text = "";
            ...
            // VALIDACIÓN EXITOSA
            MessageBox.Show(
                $"CHEQUE NO. {numeroCheque} ENCONTRADO Y VALIDADO CORRECTAMENTE.",
                "VALIDACIÓN EXITOSA",
                MessageBoxButtons.OK,
        ...
        private void BloquearCamposIniciales()
        {
            ...
            // ⭐ ISR se habilita/deshabilita según tipo de cheque (en ComboBox_SelectedIndexChanged)

            // Aplicar estilo a deshabilitados
            ConfigurarEstiloDeshabilitado(Txt_NoCheque);
            ConfigurarEstiloDeshabilitado(Txt_Li);
            ConfigurarEstiloDeshabilitado(Txt_Fin);
        ...
        {
            CrearTextBoxConPadding(Txt_ValorImpresoCheque, true);
        ...
        private void ConfigurarComboBoxSinScroll()
        {
            ...
            ComboBox_TipoCheque.MouseWheel += ComboBox_MouseWheel_Disabled;
        ...
        private void Btn_SearchBeneficiario_Click(object sender, EventArgs e)
        {
            try
            {
                Frm_Checks_SearchBeneficiario frmBuscar = new Frm_Checks_SearchBeneficiario(this);
                DialogResult resultado = frmBuscar.ShowDialog();
        ...
        private void ConfigurarTabIndexYFocus()
        {
            ...
            // Sección: Datos del Cheque
            DTP_Emision.TabIndex = 0;
            Txt_NoCheque.TabIndex = 1;
            ComboBox_TipoCheque.TabIndex = 2;
            ...
            // Botones
            Btn_AsignarCheque.TabIndex = 29;
            Btn_Imprimir.TabIndex = 30;
            Btn_ClearCuenta.TabIndex = 31;
            Btn_ClearInformacion.TabIndex = 32;
            Btn_ClearPartida.TabIndex = 33;
        ...
        // Limpiar toda la información del cheque (excepto partida)
        private void Btn_ClearInformacion_Click(object sender, EventArgs e)
        {
            var confirmacion = MessageBox.Show(
                "¿DESEA LIMPIAR TODA LA INFORMACIÓN DEL CHEQUE?",
                "CONFIRMAR LIMPIEZA",
                MessageBoxButtons.YesNo,
                MessageBoxIcon.Question);
            ...
            {
                LimpiarInformacionCheque();
            }
        }

        private void LimpiarInformacionCheque()
        {
            Txt_Beneficiario.Text = "NOMBRE DEL BENEFICIARIO";
            Txt_Beneficiario.ForeColor = Color.Gray;
            Txt_Concepto.Text = "CONCEPTO DEL CHEQUE";
        ...
        private void ConfigurarFX890IIMaximaCalidad(PrinterSettings printerSettings)
        {
            try
            {
                ...
                printDocument.DocumentName = "Cheque_FX890II_MaxCalidad";
            }
            catch (Exception ex)
            {
        ...
        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            RectangleF rectEntidad = new RectangleF(tabulacion, currentY, anchoDisponibleEntidad, 40);
            ...
            fontEntidad.Dispose(); // Liberar recurso
            currentY += 25;

            // ===== CAMBIO 1: Monto en letras con autoajuste =====
            // Calcular el ancho disponible para MontoLetras
            float anchoDisponibleMontoLetras = (rightMargin - 65) - tabulacion; // Hasta donde está el monto numérico
            RectangleF rectMontoLetras = new RectangleF(tabulacion, currentY, anchoDisponibleMontoLetras, 40);

            // Calcular tamaño óptimo de fuente
            Font fontMontoLetrasTemp = CalcularFuenteOptima(g, datosRecibo.MontoLetras, rectMontoLetras, 12f, 9f);
            // Crear nueva fuente asegurando que sea Regular (sin negrita)
            Font fontMontoLetras = new Font("Calibri Light", fontMontoLetrasTemp.Size, FontStyle.Regular);
            fontMontoLetrasTemp.Dispose(); // Liberar la fuente temporal

            g.DrawString(datosRecibo.MontoLetras, fontMontoLetras, brush, tabulacion, currentY);
            fontMontoLetras.Dispose(); // Liberar recurso
            currentY += 80;

            // ===== CAMBIO 4 CORREGIDO: Emitido con autoajuste =====
            string textoEmitido = $"EMITIDO:    {datosRecibo.Concepto}";
            // Calcular el ancho disponible desde tabulacion2 hasta el margen derecho
            float anchoDisponibleEmitido = rightMargin - tabulacion2 - 20; // Reducido margen a solo 20 pixeles
            RectangleF rectEmitido = new RectangleF(tabulacion2, currentY, anchoDisponibleEmitido, 60);

            // Calcular tamaño óptimo de fuente (aumentado mínimo de 7f a 8.5f)
            Font fontEmitidoTemp = CalcularFuenteOptima(g, textoEmitido, rectEmitido, 10f, 8.5f);
            // Crear nueva fuente asegurando que sea Regular (sin negrita)
            Font fontEmitido = new Font("Calibri Light", fontEmitidoTemp.Size, FontStyle.Regular);
            fontEmitidoTemp.Dispose(); // Liberar la fuente temporal

            g.DrawString(textoEmitido, fontEmitido, brush, tabulacion2, currentY);
            fontEmitido.Dispose(); // Liberar recurso
            currentY += 70;
            ...
            RectangleF rectYoEntidad = new RectangleF(tabulacion2, currentY, anchoDisponibleYo, 40);
            // Calcular tamaño óptimo de fuente
            ...
            fontYoEntidad.Dispose(); // Liberar recurso
            currentY += 20;

            g.DrawString($"RECIBÍ DE:     {datosRecibo.RecibidoDe}", fontNormal, brush, tabulacion2, currentY);
            currentY += 20;

            // ===== CAMBIO 2: LA CANTIDAD DE con autoajuste =====
            string textoCantidad = $"LA CANTIDAD DE:    {datosRecibo.MontoLetras}";
            // Calcular el ancho disponible desde tabulacion2 hasta el margen derecho
            float anchoDisponibleCantidad = rightMargin - tabulacion2 - 50; // 50 pixeles de margen de seguridad
            RectangleF rectCantidad = new RectangleF(tabulacion2, currentY, anchoDisponibleCantidad, 40);

            // Calcular tamaño óptimo de fuente
            Font fontCantidadTemp = CalcularFuenteOptima(g, textoCantidad, rectCantidad, 12f, 8f);
            // Crear nueva fuente asegurando que sea Regular (sin negrita)
            Font fontCantidad = new Font("Calibri Light", fontCantidadTemp.Size, FontStyle.Regular);
            fontCantidadTemp.Dispose(); // Liberar la fuente temporal

            g.DrawString(textoCantidad, fontCantidad, brush, tabulacion2, currentY);
            fontCantidad.Dispose(); // Liberar recurso
            currentY += 25;

            // ===== CAMBIO 3 CORREGIDO: CONCEPTO con autoajuste =====
            string textoConcepto = $"CONCEPTO: {datosRecibo.Concepto}";
            // Calcular el ancho disponible desde tabulacion2 hasta el margen derecho
            float anchoDisponibleConcepto = rightMargin - tabulacion2 - 20; // Reducido margen a solo 20 pixeles
            RectangleF rectConcepto = new RectangleF(tabulacion2, currentY, anchoDisponibleConcepto, 60);

            // Calcular tamaño óptimo de fuente (aumentado mínimo de 7f a 8.5f)
            Font fontConceptoTemp = CalcularFuenteOptima(g, textoConcepto, rectConcepto, 10f, 8.5f);
            // Crear nueva fuente asegurando que sea Regular (sin negrita)
            Font fontConcepto = new Font("Calibri Light", fontConceptoTemp.Size, FontStyle.Regular);
            fontConceptoTemp.Dispose(); // Liberar la fuente temporal

            g.DrawString(textoConcepto, fontConcepto, brush, tabulacion2, currentY);
            fontConcepto.Dispose(); // Liberar recurso
            currentY += 25;
        ...
        private void Btn_Imprimir_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // VALIDAR QUE TENGA CHEQUES DISPONIBLES
                if (Txt_NoCheque.Text == "0" || string.IsNullOrEmpty(Txt_NoCheque.Text))
                {
                    MessageBox.Show(
                        "NO TIENE CHEQUES ASIGNADOS.\n" +
                        "SOLICITE AL ADMINISTRADOR LA ASIGNACIÓN DE UN RANGO.",
                ...
                var confirmacion = MessageBox.Show(
                    "¿DESEA FINALIZAR EL PROCESO DEL CHEQUE EN EL SISTEMA?",
                    "CONFIRMAR FINALIZACIÓN",
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question);
                ...
                // Capturar datos del cheque ANTES de registrar
                string numeroChequeActual = Txt_NoCheque.Text;
                string beneficiarioActual = ObtenerTextoReal(Txt_Beneficiario).ToUpper();
                decimal valorImpresoActual = ObtenerValorDecimal(Txt_ValorImpresoCheque);
                string conceptoActual = ObtenerTextoReal(Txt_Concepto).ToUpper();

                // ===== REGISTRAR EN BASE DE DATOS =====
                if (RegistrarChequeCompleto())
                {
                    MessageBox.Show("ÉXITO AL FINALIZAR LA PARTIDA CONTABLE", "ÉXITO",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);

                    // Enviar correo con los datos capturados previamente
                    EnviarCorreoNotificacion(numeroChequeActual, beneficiarioActual, valorImpresoActual, conceptoActual);

                    // Limpiar formulario
                    LimpiarFormularioCompleto();

                    // Actualizar control de cheques
                    CargarControlCheques();
                }
            ...
            {
                this.Cursor = Cursors.Default;
                MessageBox.Show($"ERROR AL FINALIZAR CHEQUE: {ex.Message}", "ERROR",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private bool ValidarDatosCompletos()
        {
            ...
            if (CheckBox_Complemento.Checked)
            {
                ...
                {
                    MessageBox.Show(
                        "DEBE INGRESAR EL NÚMERO DEL CHEQUE COMPLEMENTO.",
                ...
                // Verificar si el cheque complemento existe
                string numeroComplemento = Txt_Complemento.Text.Trim();
                Mdl_Checks chequeComplemento = Ctrl_Checks.ObtenerChequePorNumero(numeroComplemento);

                if (chequeComplemento == null)
                {
                    MessageBox.Show(
                        $"EL CHEQUE NO. {numeroComplemento} NO EXISTE EN EL SISTEMA.",
                        "VALIDACIÓN DE COMPLEMENTO",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
                    Txt_Complemento.Focus();
                ...
                // Verificar si el cheque ya está marcado como completado
                if (chequeComplemento.LastComplement)
                {
                    MessageBox.Show(
                        $"EL CHEQUE NO. {numeroComplemento} YA ESTÁ MARCADO COMO COMPLETADO.\n" +
                        "NO SE PUEDEN AGREGAR MÁS COMPLEMENTOS A ESTE CHEQUE.",
                        "VALIDACIÓN DE COMPLEMENTO",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
            ...
            {
                MessageBox.Show(
                    "NO PUEDE MARCAR 'ÚLTIMO COMPLEMENTO' SIN UN COMPLEMENTO ACTIVO.",
                    "VALIDACIÓN",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
                CheckBox_LastComplement.Checked = false;
            ...
            // NUEVA VALIDACION: Si es LastComplement, verificar que no quede negativo
            if (CheckBox_LastComplement.Checked && CheckBox_Complemento.Checked && !string.IsNullOrWhiteSpace(Txt_Complemento.Text))
            {
                decimal valorImpreso = ObtenerValorDecimal(Txt_ValorImpresoCheque);
                ...
                {
                    MessageBox.Show(
                        $"ERROR: EL VALOR IMPRESO DEL CHEQUE ES NEGATIVO (Q.{valorImpreso:N2}).\n\n" +
                        "ESTO SIGNIFICA QUE LOS COMPLEMENTOS SUMAN MÁS QUE EL MONTO TOTAL.\n" +
                        "NO SE PUEDE CONTINUAR CON ESTA OPERACIÓN.\n\n" +
                        "VERIFIQUE LOS MONTOS DE LOS CHEQUES VINCULADOS.",
                        "FONDOS INSUFICIENTES",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    return false;
                ...
                // Validacion adicional: Verificar que el monto total cubra todos los complementos
                ...
                if (sumaComplementos > montoTotal)
                {
                    MessageBox.Show(
                        ...
                        $"MONTO TOTAL DEL CHEQUE: Q.{montoTotal:N2}\n" +
                        $"SUMA DE COMPLEMENTOS: Q.{sumaComplementos:N2}\n" +
                        $"DIFERENCIA: Q.{(montoTotal - sumaComplementos):N2}\n\n" +
                        "LOS COMPLEMENTOS NO PUEDEN SUMAR MÁS QUE EL MONTO TOTAL.",
                        "FONDOS INSUFICIENTES",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    return false;
        ...
        private bool RegistrarChequeCompleto()
        {
            try
            {
                // ===== 1. REGISTRAR CHEQUE =====
                Mdl_Checks cheque = new Mdl_Checks
                {
                    CheckNumber = Txt_NoCheque.Text,
                    IssueDate = DTP_Emision.Value,
                    IssuePlace = "GUATEMALA",
                    Amount = ObtenerValorDecimal(Txt_MontoTotal),
                    PrintedAmount = ObtenerValorDecimal(Txt_ValorImpresoCheque),
                    ...
                    WithholdingTax = ObtenerValorDecimal(Txt_ITH),
                    Retention = ObtenerValorDecimal(Txt_RetencionISR),
                    ...
                    Aguinaldo = ObtenerValorDecimal(Txt_Aguinaldo),  // NUEVO
                    Bill = "NO ASIGNADA",
                    LastComplement = false,  // Siempre empieza en false
                    PurchaseOrderNumber = ObtenerTextoReal(Txt_OrdenCompra).ToUpper(),
                    Complement = CheckBox_Complemento.Checked ? Txt_Complemento.Text : null,
                    IsActive = true,
                    CreatedBy = UserData.UserId
                };

                if (Ctrl_Checks.RegistrarCheque(cheque) == 0)
                {
                    MessageBox.Show("ERROR AL REGISTRAR CHEQUE", "ERROR",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }

                Mdl_Checks ChequeImpreso = Ctrl_Checks.ObtenerChequePorNumero(Txt_NoCheque.Text);

                // ===== 1.5. MARCAR LASTCOMPLEMENT SI ESTÁ ACTIVADO =====
                if (CheckBox_LastComplement.Checked && CheckBox_Complemento.Checked && !string.IsNullOrWhiteSpace(Txt_Complemento.Text))
                {
                    string numeroComplemento = Txt_Complemento.Text.Trim();
                    ...
                    {
                        MessageBox.Show(
                            $"ADVERTENCIA: No se pudo marcar el cheque {numeroComplemento} como completado.",
                            "ADVERTENCIA",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Warning);
                    }
                ...
                Mdl_AccountingEntryMaster partida = new Mdl_AccountingEntryMaster
                {
                    ...
                    Concept = ObtenerTextoReal(Txt_Concepto).ToUpper(),
                    StatusId = 1, // POR EJEMPLO: 1 = REGISTRADA / AUTORIZADA (según tu catálogo)
                ...
                if (partidaId == 0)
                {
                    MessageBox.Show("ERROR AL REGISTRAR PARTIDA", "ERROR",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }

                // ===== 2.1 VINCULAR PARTIDA CON CHEQUE EN AccountingEntryChecks =====
                bool vinculoOk = Ctrl_AccountingEntryChecks.RegistrarVinculo(partidaId, ChequeImpreso.CheckId);
                if (!vinculoOk)
                {
                    MessageBox.Show("ERROR AL VINCULAR LA PARTIDA CON EL CHEQUE", "ERROR",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }

                // ===== 3. REGISTRAR DETALLES DE PARTIDA =====
                foreach (DataGridViewRow row in Tabla.Rows)
                {
                ...
                // ===== 4. ACTUALIZAR CONTROL DE CHEQUES (SOLO EL RANGO USADO) =====
                if (_checkControlIdActual.HasValue)
                {
                    ...
                    if (actualizado > 0)
                    {
                        ...
                        Mdl_CheckControl controlActualizado = Ctrl_CheckControl.ObtenerControlPorId(checkControlId);

                        // VERIFICAR SI SOBREPASÓ EL LÍMITE
                        if (controlActualizado != null &&
                            controlActualizado.CurrentCounter > controlActualizado.FinalLimit)
                        {
                            ...
                            if (eliminado)
                            {
                                // (Opcional) Verificar si hay rango compartido disponible
                                Mdl_CheckControl controlCompartido = null;
                                Mdl_Users usuarioCompartido = Ctrl_Users.ObtenerUsuarioPorUsername("SYS_CHECKS_SHARE");
                                if (usuarioCompartido != null)
                                {
                                    controlCompartido = Ctrl_CheckControl.ObtenerControlPorUsuario(usuarioCompartido.UserId);
                                }
                            }
                        }
                    }
        ...
        private void RegistrarAuditoria()
        {
            try
            {
                string tipoCheque = ComboBox_TipoCheque.SelectedItem?.ToString() ?? "";
                string detalle = $"REGISTRO DE CHEQUE {tipoCheque} POR {UserData.Username.ToUpper()}, " +
                                $"EN BENEFICIO DE {ObtenerTextoReal(Txt_Beneficiario).ToUpper()}, " +
                                $"CON UN VALOR DE Q.{ObtenerValorDecimal(Txt_ValorImpresoCheque):N2}";

                Ctrl_Audit.RegistrarAccion(
                    UserData.UserId,
                    $"EMISIÓN DE CHEQUE NO. {Txt_NoCheque.Text}",
                    "Checks",
                    Convert.ToInt32(Txt_NoCheque.Text),
                    detalle
                );
            }
            catch { }
        ...
        // Cargar control de cheques del usuario
        // Cargar control de cheques del usuario
        public void CargarControlCheques()
        {
            try
            {
                // Reiniciar el control actual usado por el formulario
                _checkControlIdActual = null;

                List<Mdl_CheckControl> controles = new List<Mdl_CheckControl>();

                // ⭐ OBTENER TODOS LOS CONTROLES DEL SISTEMA
                List<Mdl_CheckControl> todosLosControles = Ctrl_CheckControl.MostrarControles();

                // ⭐ OBTENER UserId del compartido
                Mdl_Users usuarioCompartido = Ctrl_Users.ObtenerUsuarioPorUsername("SYS_CHECKS_SHARE");
                int userIdCompartido = usuarioCompartido?.UserId ?? 0;

                // ⭐ FILTRAR: Solo controles propios o compartidos
                controles = todosLosControles
                    .Where(c => c.UserId == UserData.UserId || c.UserId == userIdCompartido)
                    .ToList();

                // ⭐ ORDENAR POR PRIORIDADES:
                // 1. Propio + Priority=true (UserId propio, Priority alto)
                // 2. Compartido + Priority=true
                // 3. Propio + Priority=false
                // 4. Compartido + Priority=false
                Mdl_CheckControl controlSeleccionado = controles
                    .OrderByDescending(c => c.Priority ? 1 : 0)              // PRIMERO: prioridad alta
                    .ThenByDescending(c => (c.UserId == UserData.UserId ? 1 : 0)) // LUEGO: propios sobre compartidos
                    .FirstOrDefault();

                // ⭐ VALIDAR Y MOSTRAR
                if (controlSeleccionado != null)
                {
                    // Guardar el rango que REALMENTE está usando este formulario
                    _checkControlIdActual = controlSeleccionado.CheckControlId;

                    Txt_NoCheque.Text = controlSeleccionado.CurrentCounter.ToString();
                    Txt_Li.Text = controlSeleccionado.InitialLimit.ToString();
                    Txt_Fin.Text = controlSeleccionado.FinalLimit.ToString();

                    // ⭐ VALIDAR LÍMITE SOLO UNA VEZ
                    if (controlSeleccionado.CurrentCounter > controlSeleccionado.FinalLimit && !_advertenciaLimiteMostrada)
                    {
                        _advertenciaLimiteMostrada = true;
                        MessageBox.Show(
                            "HA ALCANZADO EL LÍMITE DE CHEQUES ASIGNADOS.\n" +
                            "CONTACTE AL ADMINISTRADOR PARA NUEVA ASIGNACIÓN.",
                            "LÍMITE ALCANZADO",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Warning);
            ...
            {
                System.Diagnostics.Debug.WriteLine($"ERROR AL CARGAR CONTROL: {ex.Message}");
            }
        }
        ...
        private void ActualizarDatosRecibo()
        {
            ...
            decimal valorImpresoFinal = ObtenerValorDecimal(Txt_ValorImpresoCheque);
                    ...
                    if (montoBaseReal > 0)
                        datosRecibo.Items.Add(new ItemImpresion
                        {
                            ...
                            Descripcion = CheckBox_LastComplement.Checked ? "MONTO TOTAL" : 
                                    (tipoChecked == "ANTICIPOS" ? "ANTICIPOS" : "DEVOLUCIÓN DE CUOTAS"),
                            Signo = "",
                            Monto = montoBaseReal.ToString("N2")
                        });
            ...
            // ===== FUNCIONALIDAD LASTCOMPLEMENT - APLICA PARA CUALQUIER TIPO DE CHEQUE =====
            // SI LASTCOMPLEMENT ESTA ACTIVADO, AGREGAR CHEQUES COMPLEMENTARIOS AL FINAL
            if (CheckBox_LastComplement.Checked && CheckBox_Complemento.Checked && !string.IsNullOrWhiteSpace(Txt_Complemento.Text))
            {
                string numeroComplemento = Txt_Complemento.Text.Trim();

                // Obtener el cheque principal primero
                Mdl_Checks chequePrincipal = Ctrl_Checks.ObtenerChequePorNumero(numeroComplemento);

                // Obtener todos los cheques vinculados a este cheque principal
                List<Mdl_Checks> chequesComplemento = Ctrl_Checks.ObtenerChequesComplemento(numeroComplemento);

                // Verificar si hay datos que mostrar
                bool hayDatosParaMostrar = (chequePrincipal != null) || (chequesComplemento != null && chequesComplemento.Count > 0);
                ...
                if (hayDatosParaMostrar)
                {
                    ...
                    // Mostrar el cheque principal primero (el que todos referencian)
                    // CORRECCION: Mostrar como RESTA con signo (-)
                    if (chequePrincipal != null)
                    {
                        datosRecibo.Items.Add(new ItemImpresion
                        {
                            Descripcion = $"ANTICIPO EN CHEQUE NO. {chequePrincipal.CheckNumber}",
                            Signo = "(-)",  // CAMBIADO: Ahora muestra que se RESTA
                            Monto = chequePrincipal.PrintedAmount.ToString("N2")
                        });
                    }

                    // Luego mostrar todos los cheques vinculados a este principal
                    // CORRECCION: Mostrar como RESTA con signo (-)
                    if (chequesComplemento != null && chequesComplemento.Count > 0)
                    {
                        foreach (var chequeComp in chequesComplemento)
                        {
                            datosRecibo.Items.Add(new ItemImpresion
                            {
                                Descripcion = $"ANTICIPO EN CHEQUE NO. {chequeComp.CheckNumber}",
                                Signo = "(-)",  // CAMBIADO: Ahora muestra que se RESTA
                                Monto = chequeComp.PrintedAmount.ToString("N2")
                            });
                        }
                    }
                }
        ...
        #endregion FinalizarCheque
        #region AsignarCheque
        private void Btn_AsignarCheque_Click(object sender, EventArgs e)
        {
            try
            {
                this.Cursor = Cursors.WaitCursor;

                // Abrir formulario de asignación de cheques como MODAL
                ...
                if (resultado == DialogResult.OK)
                {
                    ...
                    // Recargar control de cheques después de asignar
                    CargarControlCheques();

                    MessageBox.Show(
                        "RANGO DE CHEQUES ACTUALIZADO CORRECTAMENTE",
                        "ÉXITO",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
                }
        ...
        private void CalcularValoresConExencionManual()
        {
            try
            {
                ...
                // CORRECCION: Calcular suma de complementos si LastComplement esta activo
                decimal sumaComplementos = 0;
                if (CheckBox_LastComplement.Checked && CheckBox_Complemento.Checked && !string.IsNullOrWhiteSpace(Txt_Complemento.Text))
                {
                    string numeroComplemento = Txt_Complemento.Text.Trim();
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"Error al obtener complementos en exencion manual: {ex.Message}");
                    }
                }
